      ...
      if (isMultiTx) {
        ...
        for await (const res of result) {
          ...
          if (res.status === 'fulfilled') {
            success.push(
              `<div>Purchase successfully ${Number(Big(listingList?.result[index].available).toFixed(4)).toLocaleString('en-US')} Carbon: <a class="underline" href="https://explorer.solana.com/tx/${res?.value?.tx}${env.NEXT_PUBLIC_MODE === 'prod' ? '' : '?cluster=devnet'}" rel="noopener noreferrer" target="_blank">View transaction</a></div>`,
            );
          }
          index++;
        }
